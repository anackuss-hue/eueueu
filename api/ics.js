...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!r.ok) return res.status(502).send("O calendário respondeu com erro " + r.status + ". Confira se o link está certo.");
    const text = await r.text();
    if (!/BEGIN:VCALENDAR/i.test(text)) return res.status(502).send("O link não devolveu um calendário (.ics).");
